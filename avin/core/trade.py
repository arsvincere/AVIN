...
class TradeList:  # {{{
    ...
    async def save(cls, tlist) -> None:
        ...
        await Keeper.delete(tlist)  # XXX: нельзя так делать, см ниже
        await Keeper.add(tlist)

        # XXX: вот надо хорошо подумать и решить...
        # глупо сейчас получается. При записи тлист удаляет старый трейд лист
        # и удаляются и трейды и ордера и операции...
        # а потом сохраняется только сам пустой трейд лист...
        # надо или полностью все перезаписывать или вообще уходить от
        # этой упрощенной модели... и работать с этим всем как с БД
        # через апдейты...

        # может не самое оптимальное решение... но вот сделан
        # метод deleteTrades его Test использует для очистки себя
        # при перезапуске
        # так вот может им и пользоваться явно когда удалять явно
        # трейды
        # а сохранение оставить сохранением...
        # тогда вообще и в других местах проверь может там тоже
        # пагубно это удаление перед сохранением в alist например...
        # хотя там врядли..
        # тут вся тема в том что трейды сохраняются отдельно
        # по факту создания а не трейд листом...
        # а удаляться они пытаются и так и эдак, вот в чем косяк
        # если создаешь трейды отдельно, то и удалять трейды
        # надо отдельно
